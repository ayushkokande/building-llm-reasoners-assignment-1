...
# Ensure project root is on path so we can import from tests
...
# Adapters (run_get_batch, optimizer, checkpointing, LR schedule, loss, grad clip)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ctx = run_scaled_dot_product_attention(Q, K, V, mask=causal)
        ctx = ctx.transpose(1, 2).reshape(B, T, C)
        attn_out = self.attn_output_proj(ctx)
        x = x + attn_out
